...
class TCPLoggingHandler extends LoggingHandler {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) {
            Tr.event(ctx.channel(), tc, "read (async) called for local: " + ctx.channel().localAddress() + " remote: " + ctx.channel().remoteAddress() + " bytes read: "
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) {
            Tr.event(ctx.channel(), tc, "write (async) requested for local: " + ctx.channel().localAddress() + " remote: " + ctx.channel().remoteAddress()
                                        + (msg instanceof ByteBuf ? " bytes to be written: " + ((ByteBuf) msg).readableBytes() : " object to write: " + msg));
        }
        ...
        ctx.write(msg, promise).addListener(new ChannelFutureListener() {
            ...
            public void operationComplete(ChannelFuture future) throws Exception {
                if (future.isSuccess() && TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled())
                    Tr.event(ctx.channel(), tc, "write (async) finished sucessfully for local: " + ctx.channel().localAddress() + " remote: " + ctx.channel().remoteAddress()
                                                + " wrote: " + bytesToWrite + " bytes");
                else if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled())
                    Tr.event(ctx.channel(), tc, "write (async) FAILED for local: " + ctx.channel().localAddress() + " remote: " + ctx.channel().remoteAddress()
