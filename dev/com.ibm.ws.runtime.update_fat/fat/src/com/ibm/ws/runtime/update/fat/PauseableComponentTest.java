...
public class PauseableComponentTest {
     ...
     * This method forcefully terminates the server process after detecting hang warnings,
     * preventing indefinite waits during server stop.
     *
    ...
    private void startStopServerWithTimeout(TestType type) throws Exception {
        ...
        if (type == TestType.PAUSEABLE_HANG) {
            // Start the stop process in a separate thread to monitor it
            Thread stopThread = new Thread(() -> {
                try {
                    server.stopServer(false, PAUSEABLE_COMPONENT_HUNG_WARNING, PAUSEABLE_FAILURE_WARNING);
                } catch (Exception e) {
                    Log.info(c, "stopThread", "Exception during server stop: " + e.getMessage());
                }
            });
            stopThread.start();
            ...
            // Give the server a brief moment to attempt graceful shutdown after warnings
            Thread.sleep(5000);

            // If the server is still running, forcefully terminate it
            if (server.isStarted()) {
                Log.info(c, method.getMethodName(), "Server still running after hang warnings detected. Forcefully terminating...");
                server.stopServer(false, true);
            }
